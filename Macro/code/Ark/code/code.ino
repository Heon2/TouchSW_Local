...
void setup()
{
  ...
  sinmi_count = millis();
  infinity_count = millis();
  grandis_count = millis();
  magic_count = millis();
  dice_count = millis();
  exp_count = millis();
  //exp120_count = millis();
  ...
  infinity_rand_sec = random(infinity_cooltime_min, infinity_cooltime_max);
  grandis_rand_sec = random(grandis_cooltime_min, grandis_cooltime_max);
  magic_rand_sec = random(magic_cooltime_min, magic_cooltime_max);
...
void loop()
{
  ...
  // 인피 실행 체크
  if(( millis() - infinity_count ) > infinity_rand_sec )
  {
      infinity();
      infinity_rand_sec = random(infinity_cooltime_min, infinity_cooltime_max);
      infinity_count = millis();
  ...
  // 래서풀 실행 체크
  if(( millis() - sinmi_count ) > magic_rand_sec )
  {
      magic();
      magic_rand_sec = random(magic_cooltime_min, magic_cooltime_max);
      magic_count = millis();
  ...
  // 그여축 실행 체크
  if(( millis() - sinmi_count ) > grandis_rand_sec )
  {
      grandis();
      grandis_rand_sec = random(grandis_cooltime_min, grandis_cooltime_max);
      grandis_count = millis();
