...
typedef struct
{
	...
	tCell_t wacom_hover_test_point[5];
	uint8_t wacom_hover_DeltaChange_MaxDelta_Th;
#endif
#if (WACOM_HOVER_TEST_POINT_SMOOTH == YES)
	tXY_t wacom_hover_test_coordi[5];
	tXY_t pass_bound;
	int8_t point_idx;
	uint8_t counti_cnt;
	uint8_t wacom_hover_Smooth_MaxDelta_Th;
#endif
#if (PenContacctReliableCheck_ON == YES)
	uint8_t ucPenContacctReliableCheckCnt;
#endif
} __PACKED tPenInfo_t;
...
//    bool_t bWasPalm;
	...
	uint32_t ulIdleToActiveFrameCntTHD;
	uint32_t ulIdleToActiveCheckCount;
#endif
	...
	bool_t bIsESDGhostRecoveryATFirstFrame_Check;
	uint32_t ulESDGhostRecoveryATFirstFrame_Check_Count;
	uint32_t ulESDGhostRecoveryATFirstFrame_TouchExpect_Count;
#endif /* USED_ESD_RECOVERY_RESET_TOUCH_AT_FIRST_FRAME */

#if SpreadPannelDeltaDown_En
	int16_t ucDeltaDownOffset;
	uint8_t ucDeltaDownCnt;
#endif
} tAlgorithmInfo_t;
